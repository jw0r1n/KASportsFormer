...
class Attention(nn.Module):
    ...
    def forward(self, fv, fe):
        B, N, C = fv.shape
        B, E, C = fe.shape
        q = self.to_q(fv).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        k = self.to_k(fe).reshape(B, E, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v = self.to_v(fe).reshape(B, E, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
...
class HightOrderAttentionBlock(nn.Module):
    def __init__(self,
        ...
        self.foa_block = FirstOrderAttention(in_channels, out_channels, kernel_size, A, bias=bias, adj_len=adj_len)
        ...
        if self.attention:
            self.cross_attn = Attention(
                dim=out_channels, num_heads=8, qkv_bias=True, qk_scale=None,
            ...
            for hop_i in range(self.max_hop - 1):
                ...
                for i in range(len(eval(f"self.di_graph.directed_edges_hop{hop_i + 2}"))):
    ...
    def forward(self, fv, fe):
        ...
        fep_out = (fvp[..., [c for p, c in self.di_graph.directed_edges_hop1]] - fvp[
        ...
        if self.attention:
            ...
            for hop_i in range(self.max_hop):
                if 0 == hop_i:
                    fep_hop_i = (fvp[..., [c for p, c in eval(f"self.di_graph.directed_edges_hop{hop_i + 1}")]] - fvp[
                        ..., [p for p, c in eval(f"self.di_graph.directed_edges_hop{hop_i + 1}")]]).contiguous()
                    fep_hop_i = rearrange(fep_hop_i, 'N C T E -> (N T) E C')
                else:
                    joints_parts = eval(f"self.di_graph.directed_edges_hop{hop_i + 1}")
                    fep_hop_i = None
                    for part_idx, part in enumerate(joints_parts):
                        fep_part = None
                        for j in range(len(part) - 1):
                            fep = (fvp[..., part[j + 1]] - fvp[..., part[j]]).contiguous().unsqueeze(dim=-1)
                            if fep_part is None:
                                fep_part = fep
                            else:
                        ...
                        if fep_hop_i is None:
                            ...
                            fep_hop_i = torch.cat((fep_hop_i, fep_part), dim=-1)
                ...
                if fep_concat is None:
                    ...
                    fep_concat = torch.cat((fep_concat, fep_hop_i), dim=-2)
            fvp = rearrange(fvp, 'N C T V -> (N T) V C')
            fvp = self.norm_v(self.cross_attn(fvp, fep_concat)) + iden(fvp)
            fvp = self.mlp(self.norm_mlp(fvp)) + iden(fvp)  # make output joint number = adj_len
