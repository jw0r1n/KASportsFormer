...
class DataReaderWorldPose(object):
    ...
    def read_2d_wp(self):
        ...
        for i in range(trainset.shape[0]):
            trainset[i, :, :] = trainset[i, :, :] / res_w * 2 - [1, res_h / res_w]
        for i in range(testset.shape[0]):
            testset[i, :, :] = testset[i, :, :] / res_w * 2 - [1, res_h / res_w]
    ...
    def read_3d_wp(self):
        train_labels = self.data_source['train']['joint3d_image'][::self.sample_stride, :, :3].astype(np.float32)  # [N, 17, 3]
        test_labels = self.data_source['test']['joint3d_image'][::self.sample_stride, :, :3].astype(np.float32)  # [N, 17, 3]
        res_w, res_h = 1920, 1080
        for i in range(train_labels.shape[0]):
            train_labels[i, :, :2] = train_labels[i, :, :2] / res_w * 2 - [1, res_h / res_w]
            train_labels[i, :, 2:] = train_labels[i, :, 2:] / res_w * 2
        for i in range(test_labels.shape[0]):
            test_labels[i, :, :2] = test_labels[i, :, :2] / res_w * 2 - [1, res_h / res_w]
            test_labels[i, :, 2:] = test_labels[i, :, 2:] / res_w * 2
    ...
    def read_hw_wp(self):
        if self.test_hw is not None:
            return self.test_hw
        test_hw = np.zeros((len(self.data_source['test']['source']), 2))
        res_w, res_h = 1920, 1080
        for i in range(test_hw.shape[0]):
            test_hw[i] = res_w, res_h
    ...
    def get_split_id(self):
        ...
        vid_list_train = self.data_source['train']['source'][::self.sample_stride]
        vid_list_test = self.data_source['test']['source'][::self.sample_stride]
    ...
    def get_hw_wp(self):
        ...
        test_hw = self.read_hw_wp()  # train_data (1559752, 2) test_data (566920, 2)
        test_hw = self.turn_into_test_clips(test_hw)[:, 0, :]  # (N, 2)
        return test_hw

    def get_sliced_data_wp(self):
        train_data, test_data = self.read_2d_wp()  # train_data (1559752, 17, 3) test_data (566920, 17, 3)
        train_labels, test_labels = self.read_3d_wp()  # train_labels (1559752, 17, 3) test_labels (566920, 17, 3)
    ...
    def denormalize_wp(self, test_data, all_sequence=False):
