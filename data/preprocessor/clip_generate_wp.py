...
sys.path.append('../..')
sys.path.append('..')
from reader.wp_reader import DataReaderWorldPose
...
def save_clips_test(root_path, input_set, label_set, label_scaled_set, action_set, factor_set, hw_set):
...
def main():
    ...
    if args.data_type == "det":
        data_type = "dete"
        source_file = '../wp_hr_conf_cam_source_final.pkl'
    elif args.data_type == "gt":
        data_type = "gt"
        source_file = '../wp_no_conf_cam_source_final.pkl'
    else:
        raise ValueError(f"Unknown data type: {args.data_type}. Use 'det' or 'gt'.")

    worldpose_data_processor = DataReaderWorldPose(n_frames=n_frames, sample_stride=1, data_stride_train=n_frames // 3,
                                                   data_stride_test=n_frames,
                                                   source_file_path = source_file)
    train_dict, test_dict = worldpose_data_processor.get_sliced_data_wp()
    ...
    root_path = f"../clips/WP{data_type}-{n_frames}"
    ...
    save_clips_test(root_path=root_path, input_set=test_dict["data"], label_set=test_dict["label"], label_scaled_set=test_dict["label_scaled"],
                    action_set=test_dict["action"], factor_set=test_dict["factor"], hw_set=test_dict["test_hw"])
