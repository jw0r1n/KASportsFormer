...
def evaluate_one_epoch_new(args_dict, model, test_loader, device, epoch_index, logwriter):
    ...
    with torch.no_grad():  # 加上了这个以后这里的都是Tensor
        for joint_input, joint_label, joint_label_scaled, joint_factor, joint_action, joint_res in tqdm(test_loader,
            ...
            joint_label = joint_label.to(device)
            ...
            if args_dict.flip:
                ...
                batch_label_flip = joint_flip(joint_label)
                predicted_pos_original = model(joint_input, joint_label)
                predicted_pos_flip = model(batch_input_flip, batch_label_flip)
...
def evaluate(args_dict: EasyDict) -> None:
    ...
    logwriter = get_logger(args_dict.logger_dir_path, f"{args_dict.logger_file_name}_evaluate.log")
...
def train(args_dict: EasyDict) -> None:
    ...
    for epoch in range(epoch_start, args_dict.epochs):
        ...
        loss_record_names = ['loss_mpjpe', 'loss_total']
