...
from utils.loss_calc import (mpjpe_loss_calc, n_mpjpe_loss_calc, velocity_loss_calc, loss_limb_var_calc,
                             loss_limb_len_calc, loss_cos_simi_calc, loss_cos_simi_velocity_calc, weighted_mpjpe, mean_velocity_error_train, Norm_Loss)
...
def evaluate_one_epoch_new(args_dict, model, test_loader, device, epoch_index, logwriter):
    ...
    # print("in evaluation new")

    model.eval()

    # mpjpe_all = np.zeros(total_test_frame_numbers)
    # jpe_all_joints = np.zeros((total_test_frame_numbers, args_dict.num_joints))
    # p_mpjpe_all = np.zeros(total_test_frame_numbers)
    # acc_err_all = np.zeros(total_test_frame_numbers)
    # overlay_count = np.zeros(total_test_frame_numbers)
    ...
    with torch.no_grad():  # 加上了这个以后这里的都是Tensor
        for joint_input, joint_label_scaled, joint_factor, joint_action, joint_res in tqdm(test_loader,
            ...
            for idx, predicted_data in enumerate(predicted_result):  # de-normalization
                ...
                # TODO: 这边开始可以改， 想一个overlay可以用来记录出现的frame的index
    ...
    # TODO: 换另一种做法，直接遍历每一个key, 对每一个key做np.mean
    ...
    # time_end = time.time()
    # print(f"\033[5;34;46m evaluation time cost{time_end - time_start} \033[0m")

    action_result_log_message = ""
    for action_index, action_name in enumerate(action_name_sequence):
        # print(action_name + ": " + str(final_result_mpjpe_per_action[action_index]))
        action_result_log_message = action_result_log_message + "\n" + action_name + ": " + str(final_result_mpjpe_per_action[action_index])
        # logwriter.info(action_name + ": " + str(final_result_mpjpe_per_action[action_index]))
    logwriter.info(action_result_log_message)


    joint_result_log_message = ""
    for joint_idx in range(args_dict.num_joints):
        # print("joint_idx: " + str(joint_idx) + " " + H36M_JOINT_TO_LABEL[joint_idx] + " " + str(joint_errors_count_for_all[joint_idx]))
        ...
        # logwriter.info("joint_idx: " + str(joint_idx) + " " + H36M_JOINT_TO_LABEL[joint_idx] + " " + str(joint_errors_count_for_all[joint_idx]))
...
def evaluate(args_dict: EasyDict) -> None:
    ...
    # sports_data_processor = DataReaderSportsPose(n_frames=args_dict.n_frames, sample_stride=1,
    #                                              data_stride_train=args_dict.n_frames // 3, data_stride_test=args_dict.n_frames,
    #                                              source_file_path=args_dict.source_file_path)
...
def train_one_epoch(args_dict, model, train_loader: DataLoader, optimizer, device, epoch_losses_dict, epoch_index, traj_loss_fn) -> None:
    model.train()
    ...
    for x, y in train_progress_bar:
        ...
        loss_traj = traj_loss_fn(predict_result, y)
        ...
        loss_total = loss_mpjpe + 0.1 * loss_traj

        epoch_losses_dict['loss_mpjpe'].update(loss_mpjpe.item(), batch_size)
        epoch_losses_dict['loss_traj'].update(loss_traj.item(), batch_size)
        ...
        train_progress_bar.set_postfix({
            ...
            'loss_traj': epoch_losses_dict['loss_traj'].avg,
...
def train(args_dict: EasyDict) -> None:
    ...
    test_loader = DataLoader(test_dataset, shuffle=True, batch_size=args_dict.batch_size, num_workers=args_dict.num_cpus - 1,
                              pin_memory=args_dict.pin_memory, prefetch_factor=args_dict.num_cpus // 3,
                              persistent_workers=args_dict.persistent_workers)  # 现在的代码是从reader里面取结果的，所以这边的test不能开shuffle
    ...
    traj_loss_fn = Norm_Loss(vec_length_list=[8,12,16,24], eps=1e-6, normalize=False, diff_order='L2')
    traj_loss_fn = traj_loss_fn.cuda()
    ...
    for epoch in range(epoch_start, args_dict.epochs):
        ...
        loss_record_names = ['loss_mpjpe', 'loss_traj', 'loss_total']
        ...
        train_one_epoch(args_dict=args_dict, model=model, train_loader=train_loader,
                        optimizer=optimizer, device=device, epoch_losses_dict=epoch_losses_dict, epoch_index=epoch, traj_loss_fn=traj_loss_fn)
        ...
        if mpjpe < min_mpjpe:
            ...
            checkpoint_save(checkpoint_path_best, epoch, learning_rate, optimizer, model, min_mpjpe, wandb_run_id)
        else:
            training_patience_count = training_patience_count + 1
        checkpoint_save(checkpoint_path_latest, epoch, learning_rate, optimizer, model, min_mpjpe, wandb_run_id)
        ...
        if args_dict.use_wandb:
            wandb.log({
                ...
                'train/loss_traj': epoch_losses_dict['loss_traj'].avg,
        ...
        # learning_rate = decay_learning_rate_exponentially(learning_rate, learning_rate_decay, optimizer)
...
def main() -> None:
    ...
    parser.add_argument("--config-path", type=str, default="configs/sportspose-hdformer.yaml", help="path to the config file")
